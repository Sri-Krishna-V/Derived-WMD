...
export async function POST(request: NextRequest) {
  try {
    const { prompt, model = 'google/gemini-2.5-pro', context, isEdit = false } = await request.json();
...
1. Do NOT use markdown code blocks (\`\`\`jsx, \`\`\`css, etc.) inside the <file> tags.
2. The content inside <file> tags must be the RAW file content only.
3. Do NOT wrap the XML output in markdown blocks.
...
5. Make the requested change immediately`;
        ...
        const isGroqOSS = model.includes('gpt-oss');
        const isOpenAI = model.startsWith('openai/') && !isGroqOSS;
        
        const modelProvider = isAnthropic ? anthropic : (isOpenAI ? openai : (isGoogle ? googleGenerativeAI : groq));
        
        const actualModel = isAnthropic ? model.replace('anthropic/', '') : 
                           (isOpenAI && model.startsWith('openai/')) ? model.replace('openai/', '') :
